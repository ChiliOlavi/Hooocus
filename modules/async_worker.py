...
def worker():
    ...
    def save_and_log(async_task: ImageGenerationSeed, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for x in imgs:
            ...
            img_paths.append(log(x, async_task.path_outputs, d, metadata_parser, async_task.output_format, task, persist_image))
    ...
    def process_enhance(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path,
        ...
        if 'upscale' in goals:
            ...
            if direct_return:
                ...
                progressbar(async_task, current_progress, f'Saving image {current_task_id + 1}/{total_count} to system ...')
                uov_image_path = log(img, async_task.path_outputs, d, output_format=async_task.output_format, persist_image=persist_image)
    ...
    def handler(async_task: ImageGenerationSeed):
        ...
        if 'upscale' in goals:
            ...
            if direct_return:
                ...
                uov_input_image_path = log(async_task.uov_input_image, async_task.path_outputs, d, output_format=async_task.output_format)
