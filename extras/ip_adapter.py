...
SD_V12_CHANNELS = (
    [320] * 4 + [640] * 4 + [1280] * 4 + [1280] * 6 + [640] * 6 + [320] * 6 + [1280] * 2
)
SD_XL_CHANNELS = [640] * 8 + [1280] * 40 + [1280] * 60 + [640] * 12 + [1280] * 20


def _sdp(q, k, v, extra_options):
    return attention.optimized_attention(
        q, k, v, heads=extra_options["n_heads"], mask=None
    )
...
class To_KV(torch.nn.Module):
    def __init__(self, cross_attention_dim):
        ...
        self.to_kvs = torch.nn.ModuleList(
            [
                torch.nn.Linear(cross_attention_dim, channel, bias=False)
...
class IpaAdapterManagement:
    ...
    def load_ip_adapter(self, clip_vision_path, ip_negative_path, ip_adapter_path):
        ...
        use_fp16 = model_management.should_use_fp16(device=self.load_device)
        ...
        if plus:
            ...
            clip_extra_context_tokens = (
                ip_state_dict["image_proj"]["proj.weight"].shape[0] // cross_attention_dim
            )
        ...
        ip_adapter.load_device = self.load_device
        ip_adapter.offload_device = self.offload_device
        ip_adapter.dtype = torch.float16 if use_fp16 else torch.float32
        ip_adapter.to(self.offload_device, dtype=ip_adapter.dtype)

        image_proj_model = ModelPatcher(
            model=ip_adapter.image_proj_model,
            load_device=self.load_device,
            offload_device=self.offload_device,
        )
        ip_layers = ModelPatcher(
            model=ip_adapter.ip_layers,
            load_device=self.load_device,
            offload_device=self.offload_device,
        )

        self.ip_adapters[ip_adapter_path] = dict(
...
def patch_model(model: ModelPatcher, tasks: List[BaseControlNetTask]):
    ...
    def make_attn_patcher(ip_index):
        def patcher(n, context_attn2, value_attn2, extra_options):
            org_dtype = n.dtype
            current_step = float(
                model.model.diffusion_model.current_step.detach().cpu().numpy()[0]
            )
